...
(defmethod -message-handler :webrtclojure/signal
  ...
  (channel-send! connected-uids 
                [:webrtclojure/offer (get-in (:event ev-msg)[1])] 8000)
  (channel-send! :sente/all-users-without-uid 
                [:webrtclojure/offer (get-in (:event ev-msg)[1])] 8000))
...
(defmethod -message-handler :webrtclojure/answer
  ...
  (channel-send! connected-uids 
                [:webrtclojure/answer (get-in (:event ev-msg)[1])] 8000)
  (channel-send! :sente/all-users-without-uid 
                [:webrtclojure/answer (get-in (:event ev-msg)[1])] 8000))
